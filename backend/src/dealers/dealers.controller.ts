...
class UpdateDealerDto {
  ...
  // isActive toggles whether the dealer's users can log in
  // and whether the dealer shows up in the admin picker.
...
export class DealersController {
  constructor(private svc: DealersService, private prisma: PrismaService, private auth: AuthService) {}
  ...
  get(@Param('id') id: string, @CurrentUser() u: JwtPayload) {
    // Admins can read any dealer. Owners/staff can only read
    ...
    // matches what a UI navigating to /dealers/<id> expects.
    if (u.role !== Role.ADMIN && u.dealerId !== id) {
      return this.svc.get(u.dealerId!);
    }
  ...
  @Roles(Role.DEALER_OWNER)
  @Post(':id/staff')
  addStaff(@Param('id') id: string, @Body() dto: AddStaffDto, @CurrentUser() u: JwtPayload) {
    if (u.role !== Role.ADMIN && u.dealerId !== id) {
      return this.svc.addStaff(u.dealerId!, dto.email, dto.fullName, dto.password);
    }
   ...
   */
  @Roles(Role.DEALER_OWNER, Role.ADMIN)
  ...
  async resetStaffPassword(
    ...
    if (u.role !== Role.ADMIN && u.dealerId !== id) {
      throw new ForbiddenException('Cannot reset password for another dealer');
    }
    const staff = await this.prisma.user.findUnique({ where: { id: staffId } });
    if (!staff) throw new NotFoundException('Staff user not found');
    if (staff.dealerId !== id) throw new ForbiddenException('Staff does not belong to this dealer');
  ...
  async deactivateStaff(@Param('id') id: string, @Param('staffId') staffId: string, @CurrentUser() u: JwtPayload) {
    if (u.role !== Role.ADMIN && u.dealerId !== id) {
      throw new ForbiddenException('Cannot deactivate a staff member of another dealer');
    }
    const staff = await this.prisma.user.findUnique({ where: { id: staffId } });
    if (!staff) throw new NotFoundException('Staff user not found');
    if (staff.dealerId !== id) throw new ForbiddenException('Staff does not belong to this dealer');
  ...
  async reactivateStaff(@Param('id') id: string, @Param('staffId') staffId: string, @CurrentUser() u: JwtPayload) {
    if (u.role !== Role.ADMIN && u.dealerId !== id) {
      throw new ForbiddenException('Cannot reactivate a staff member of another dealer');
    }
    const staff = await this.prisma.user.findUnique({ where: { id: staffId } });
    if (!staff) throw new NotFoundException('Staff user not found');
    if (staff.dealerId !== id) throw new ForbiddenException('Staff does not belong to this dealer');
