  ...
  return (
    <div className="min-h-full">
      <header className="bg-white border-b px-4 sm:px-6 py-3 flex items-center flex-wrap gap-2">
        ...
        <span className="ml-2 text-xs text-slate-500">Your effective price per product (after any per-wholesaler override).</span>
      </header>
        ...
        <div className="bg-white border rounded overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(p => {
                ...
                return (
                  <tr key={p.id} className="border-b last:border-0">
                    ...
                    <td>{p.category}</td>
                    <td>{p.unit}</td>
                    <td className="text-xs">{(p.heightOptions || []).join(', ')}</td>
                    <td className="text-xs">{(p.colorOptions || []).join(', ')}</td>
              ...
              {!filtered.length && (
                <tr><td colSpan={8} className="text-center py-8 text-slate-500">No products match the filter.</td></tr>
              )}
            </tbody>
          </table>
        </div>
